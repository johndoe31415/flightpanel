...
void cairo_font_surface_init(struct cairo_font_surface_t *cfs, const char *fontname, double fontsize) {
	...
	cairo_font_options_set_antialias(fopts, CAIRO_ANTIALIAS_NONE);
...
int main(int argc, char **argv) {
	if (argc != 4) {
		fprintf(stderr, "%s [Fontname] [Fontsize] [Outfile]\n", argv[0]);
		exit(EXIT_FAILURE);
	}

	const char *fontname = argv[1];
	double fontsize = atof(argv[2]);
	const char *outfilename = argv[3];

	struct cairo_font_surface_t cfs;
	cairo_font_surface_init(&cfs, fontname, fontsize);
