...
void dump_instrument_data(FILE *f, const struct instrument_data_t *data) {
	...
	fprintf(f, "DME distance %d.%d nm, %d kt\n", data->dme.distance_nm_tenths / 10, data->dme.distance_nm_tenths % 10, data->dme.speed_kt);
	...
	fprintf(f, "Ident: NAV (%.8s), ADF (%.8s)\n", data->nav1.ident, data->adf.ident);
	...
	fprintf(f, "IAS %d kt, IALT %d ft, QNH %d mBar\n", data->misc.ias_kt, data->misc.indicated_alt_ft, data->misc.qnh_millibar);
}
