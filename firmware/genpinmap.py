	...
	def _write(self, f):
		...
		for pins in sorted(self._pins.values()):
			for pin in pins:
				...
				self._print_cols("#define %s_set(value)" % (pin.name), "if (value) { %s_set_HIGH(); } else { %s_set_LOW(); }" % (pin.name, pin.name), file = f)
				self._print_cols("#define %s_toggle()" % (pin.name), "%s_GPIO->ODR ^= %s_Pin" % (pin.name, pin.name), file = f)
				self._print_cols("#define %s_pulse()" % (pin.name), "do { %s_set_HIGH(); delay_loopcnt(LOOPCOUNT_50NS); %s_set_LOW(); } while (0)" % (pin.name, pin.name), file = f)
				self._print_cols("#define %s_npulse()" % (pin.name), "do { %s_set_LOW(); delay_loopcnt(LOOPCOUNT_50NS); %s_set_HIGH(); } while (0)" % (pin.name, pin.name), file = f)
